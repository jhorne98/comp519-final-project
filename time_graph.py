...
def time_neo4j_queries():
    with GraphDatabase.driver(configs.neo4j_config['uri'], auth=configs.neo4j_config['auth']) as client:
        ...
        for length in DBLength:
            for type in DBType:
                ...
                curs_time = timeit.timeit(lambda: client.execute_query(query, database_="neo4j"), number=QUERY_RUNS)
                ...
                if type is DBType.INTEGER:
                    ...
                    query = "MATCH (n:Node) WHERE n.table = '" + table_name + "' AND n.payload < 32767 RETURN COUNT(n)"
                    curs_time = timeit.timeit(lambda: client.execute_query(query, database_="neo4j"), number=QUERY_RUNS)
                    results.append((table_name, "I2", curs_time/QUERY_RUNS))
                    print(table_name, "I2")
                else:
                    query = "MATCH (n:Node) WHERE n.table = '" + table_name + "' AND n.payload CONTAINS 'asdf' RETURN COUNT(n)"
                    curs_time = timeit.timeit(lambda: client.execute_query(query, database_="neo4j"), number=QUERY_RUNS)
